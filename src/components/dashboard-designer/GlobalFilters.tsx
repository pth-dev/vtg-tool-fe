...
export default function GlobalFilters({ filters, onChange, isPreview }: Props) {
  return (
    <Box sx={{ display: 'flex', gap: 2, p: 2, bgcolor: '#fff', borderBottom: '1px solid #e0e0e0', flexWrap: 'wrap', alignItems: 'center' }}>
      ...
      <TextField
        ...
        onChange={(e) => onChange({ ...filters, dateRange: { ...filters.dateRange, end: e.target.value, start: filters.dateRange?.start || '' } })}
        disabled={isPreview}
        sx={{ width: 150 }}
      />
    </Box>
