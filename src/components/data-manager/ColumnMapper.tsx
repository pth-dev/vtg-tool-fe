...
import {
  ...
  TableHead, TableRow, Select, MenuItem, Chip, Switch, Alert
} from '@mui/material'
      ...
      {validation && (
        <Box sx={{ mb: 2 }}>
          ...
          {validation.warnings.map((warn, i) => (
            <Alert key={i} severity="warning" sx={{ mb: 1 }}>{warn}</Alert>
          ))}
              ...
              <TableRow key={col.name} sx={{ opacity: mapping[i]?.include ? 1 : 0.5 }}>
                ...
                <TableCell>
                  <Select
                    ...
                    onChange={(e) => handleTypeChange(i, e.target.value as ColumnMapping['targetType'])}
                    size="small"
                    sx={{ minWidth: 100 }}
                  >
