...
import { ArrowBack, Edit, Redo, Save, Share, Undo, Visibility } from '@mui/icons-material'
import {
  AppBar,
  Box,
  Button,
...
import { ChartPreview } from '@/features/chart-builder/components'
import {
  DashboardCanvas,
  GlobalFilters,
  PropertiesPanel,
  ...
  Widget,
  WidgetSidebar,
} from '@/features/dashboard-designer/components'
import { useAutoSave } from '@/features/dashboard-designer/hooks/useAutoSave'
import { useUndoRedo } from '@/features/dashboard-designer/hooks/useUndoRedo'
...
export default function DashboardDesignerPage() {
  ...
  const { data: chartDataMap } = useQuery({
    ...
    queryFn: async () => {
      ...
      await Promise.all(
        chartIds.map(async (cid) => {
          try {
            results[cid] = await api.getChartData(cid)
          } catch {}
        })
      )
      return results
  ...
  const saveMutation = useMutation({
    mutationFn: (data: { name: string; widgets: Widget[] }) =>
      api.updateDashboard(Number(id), data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['dashboards'] }),
  })

  const { isSaving, lastSaved } = useAutoSave(
    { name, widgets },
    (data) => saveMutation.mutateAsync(data).then(() => {}),
    5000
  )
  ...
  const updateWidget = useCallback(
    (wid: string, updates: Partial<Widget>) => {
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      <AppBar position="static" color="default" elevation={1}>
        <Toolbar variant="dense">
          <IconButton edge="start" onClick={() => navigate({ to: '/admin/dashboard-list' })}>
            <ArrowBack />
          </IconButton>
          <TextField
            value={name}
          ...
          <Button
            startIcon={<Save />}
            onClick={() => saveMutation.mutate({ name, widgets })}
