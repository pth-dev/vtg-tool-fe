...
export default function ShareDialog({
  ...
  const shareMutation = useMutation({
    mutationFn: (data: { userId: number; permission: string }) =>
      api.shareDashboard(dashboardId, data.userId, data.permission),
    onSuccess: () => {
  ...
  const publishMutation = useMutation({
    mutationFn: (data: { password?: string; expiresAt?: string }) =>
      api.publishDashboard(dashboardId, data),
        ...
        {tab === 0 && (
          <Box>
            <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
              ...
              <Button
                variant="contained"
                onClick={() => shareMutation.mutate({ userId: +userId, permission })}
                disabled={!userId}
              >
                Share
              </Button>
            ...
            <List dense>
              {shares?.map((s) => (
                <ListItem key={s.user_id}>
                  ...
                  <ListItemSecondaryAction>
                    <IconButton size="small" onClick={() => unshareMutation.mutate(s.user_id)}>
                      <Delete />
                    </IconButton>
                  </ListItemSecondaryAction>
                </ListItem>
            ...
            {isPublic ? (
              <>
                ...
                <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                  ...
                  />
                  <Button startIcon={<ContentCopy />} onClick={copyLink}>
                    {copied ? 'Copied!' : 'Copy'}
                  </Button>
                </Box>
                <Button color="error" onClick={() => unpublishMutation.mutate()}>
                  Unpublish
                </Button>
              </>
            ) : (
              <>
                ...
                <Button variant="contained" startIcon={<Link />} onClick={handlePublish}>
