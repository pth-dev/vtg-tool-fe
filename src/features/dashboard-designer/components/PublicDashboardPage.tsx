...
import { DashboardCanvas, Widget } from '@/features/dashboard-designer/components'
...
export default function PublicDashboardPage() {
  ...
  const { data: chartDataMap } = useQuery({
    queryKey: [
      'public-chartData',
      dashboard?.widgets?.filter((w) => w.chartId).map((w) => w.chartId),
    ],
    queryFn: async () => {
      const chartIds = [
        ...
        <Paper sx={{ p: 4, maxWidth: 400, width: '100%' }}>
          <Box sx={{ textAlign: 'center', mb: 3 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              Enter password to view this dashboard
            </Typography>
          </Box>
          <TextField
            fullWidth
          ...
          />
          <Button fullWidth variant="contained" onClick={() => setSubmittedPassword(password)}>
            View Dashboard
          </Button>
        </Paper>
      </Box>
